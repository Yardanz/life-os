...
type ImpactMode = "stabilize" | "overload";
type ImpactMetric = "risk" | "burnout";
type Lever = "sleep" | "stress" | "deepWork" | "workout";

type ImpactContribution = { lever: string; delta: number; label: string };
...
function applyLever(
  base: ProjectionAvgInputs,
  mode: ImpactMode,
  baselineWorkoutRate: number,
  lever?: Lever
): ProjectionAvgInputs {
  const isStabilize = mode === "stabilize";
  const withModeAll = lever === undefined;
  const shouldApply = (current: Lever) => withModeAll || lever === current;

  const next: ProjectionAvgInputs = {
    ...base,
    workoutRate: baselineWorkoutRate,
  };

  if (shouldApply("sleep")) {
    const deltaHours = isStabilize ? 1 : -1;
    next.sleepHours = isStabilize
      ? clamp(base.sleepHours + deltaHours, 0, 9)
      : clamp(base.sleepHours + deltaHours, 5, 12);
  }
  if (shouldApply("stress")) {
    next.stressLevel = isStabilize
      ? clamp(base.stressLevel - 2, 1, 10)
      : clamp(base.stressLevel + 2, 1, 10);
  }
  if (shouldApply("deepWork")) {
    next.deepWorkMinutes = isStabilize
      ? clamp(base.deepWorkMinutes * 0.6, 0, 360)
      : clamp(base.deepWorkMinutes * 1.3, 0, 360);
  }
  if (shouldApply("workout")) {
    next.workoutRate = isStabilize ? 0 : 1;
  }

  return next;
}

function leverLabel(lever: Lever): string {
  switch (lever) {
    case "sleep":
      return "Sleep timing";
    case "stress":
...
function run72hOutcome(
  context: PreparedContext,
  mode: ImpactMode,
  avgInputs: ProjectionAvgInputs,
  seedTag: string
): { riskEnd: number; burnoutEnd: number } {
  const simulation = simulateForward30d({
    userId: context.userId,
    startDate: context.startDate,
    step: "DAILY",
    inertiaStart: {
      risk: context.currentRiskSeed,
      burnoutRisk: clamp(context.currentBio.burnoutRiskIndex, 0, 100),
      strain: clamp(context.currentBio.strainIndex, 0, 100),
    },
    initialBioState: context.currentBio,
    initialStatsSnapshot: context.initialStatsSnapshot,
    avgInputs,
    scenario: "BASELINE",
    config: context.config,
    previousSnapshot: context.previousSnapshot,
    previousBioState: context.previousBio,
    previousLifeScores: context.previousLifeScores,
    calibration: context.calibration,
    seedSalt: `impact72h:${mode}:${seedTag}`,
  });

  const day3 = simulation.days[2];
  return {
    riskEnd: clamp(day3?.risk ?? 0, 0, 100),
    burnoutEnd: clamp(day3?.burnoutRisk ?? 0, 0, 100),
  };
}

export async function computeImpact(params: {
  userId: string;
  endDateISO: string;
  horizonHours: 72;
  mode: ImpactMode;
  ...
  if (!context) {
    return {
      ...
      modeEnd: 0,
      netDelta: 0,
      contributions: [{ lever: "interaction", delta: 0, label: "Interaction / overlap" }],
    };
  }

  const baselineInputs = {
    ...context.baseAvgInputs,
    workoutRate: context.baselineWorkoutRate,
  };
  const modeInputs = applyLever(context.baseAvgInputs, params.mode, context.baselineWorkoutRate, undefined);

  const baselineOutcome = run72hOutcome(context, params.mode, baselineInputs, "baseline");
  const modeOutcome = run72hOutcome(context, params.mode, modeInputs, "mode-all");

  const selectMetric = (outcome: { riskEnd: number; burnoutEnd: number }): number =>
    params.metric === "risk" ? outcome.riskEnd : outcome.burnoutEnd;

  const baseEnd = selectMetric(baselineOutcome);
  const modeEnd = selectMetric(modeOutcome);
  const netDelta = modeEnd - baseEnd;

  const leverOrder: Lever[] = ["sleep", "stress", "deepWork", "workout"];
  const leverContributions = leverOrder
    .map((lever) => {
      const onlyLeverInputs = applyLever(context.baseAvgInputs, params.mode, context.baselineWorkoutRate, lever);
      const outcome = run72hOutcome(context, params.mode, onlyLeverInputs, `only-${lever}`);
      const delta = selectMetric(outcome) - baseEnd;
      return {
        lever,
  ...
  const top3 = leverContributions.slice(0, 3).map((line) => ({
    lever: line.lever,
    delta: round1(line.delta),
    label: line.label,
  }));
  const residual = netDelta - top3.reduce((sum, item) => sum + item.delta, 0);

  return {
    horizonHours: params.horizonHours,
    metric: params.metric,
    baseEnd: round1(baseEnd),
    modeEnd: round1(modeEnd),
    netDelta: round1(netDelta),
    contributions: [
      ...top3,
      {
        lever: "interaction",
        delta: round1(residual),
