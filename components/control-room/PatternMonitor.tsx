...
const PATTERN_LABELS_RU: Record<PatternType, string> = {
  cyclical_stress_load: "Циклическая стресс-нагрузка",
  sleep_irregularity: "Нерегулярный сон",
  burnout_acceleration: "Ускорение выгорания",
  autonomic_drift: "Автономный дрейф",
  circadian_drift: "Циркадный дрейф",
};
...
export function PatternMonitor({ patterns, locale, onDetails }: PatternMonitorProps) {
  ...
  const labels = locale === "ru" ? PATTERN_LABELS_RU : PATTERN_LABELS;
    ...
    <section className="rounded-lg border border-zinc-800 bg-zinc-950/60 p-3">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-100">{modeText} ({Math.round(patterns.systemModeConfidence * 100)}%)</p>
        </div>
        <button
          type="button"
          onClick={onDetails}
      ...
      <div className="mt-3 space-y-2">
        {patterns.topPatterns.length > 0 ? (
          patterns.topPatterns.map((signal) => (
            ...
            >
              <span className="text-xs text-zinc-300">{labels[signal.type]}</span>
              <span className={`rounded px-2 py-0.5 text-xs ${severityTone(signal.severity)}`}>
                S{signal.severity}
              </span>
            </div>
          ))
